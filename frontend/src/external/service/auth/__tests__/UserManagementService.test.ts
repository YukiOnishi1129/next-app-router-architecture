...
// Mock the repository
jest.mock('@/external/repository/UserRepository');
...
describe('UserManagementService', () => {
  ...
  beforeEach(() => {
    ...
    mockRepository = (UserRepository as jest.MockedClass<typeof UserRepository>)
      .mock.instances[0] as jest.Mocked<UserRepository>;
  });
  ...
  describe('updateUser', () => {
    it('should update user profile', async () => {
      ...
      // Act
      const result = await userService.updateUser(existingUser.getId().getValue(), {
        name: 'New Name',
  ...
  describe('hasPermission', () => {
    it('should return true for admin with any permission', () => {
      ...
      expect(userService.hasPermission(adminUser, 'another.permission')).toBe(true);
    });
    ...
    it('should check specific permissions for member', () => {
      ...
      expect(userService.hasPermission(memberUser, 'request.delete')).toBe(false);
