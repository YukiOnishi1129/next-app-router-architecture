...
import { z } from 'zod';
import { CommentRepository, RequestRepository, UserRepository } from '@/external/domain';
...
export async function addComment(data: unknown): Promise<CommentResponse> {
  try {
    ...
    if (validated.parentId) {
      const parentComment = await commentRepository.findById(validated.parentId);
      if (parentComment) {
 ...
 * Delete a comment
 */
export async function deleteComment(data: unknown): Promise<{ success: boolean; error?: string }> {
  try {
    const session = await getSession();
    if (!session.isAuthenticated || !session.user) {
      return {
...
export async function getComments(data: unknown): Promise<CommentListResponse> {
  try {
    ...
    // Get comments
    const allComments = await commentRepository.findByRequestId(validated.requestId);
 ...
 */
export async function getCommentThread(commentId: string): Promise<CommentListResponse> {
  try {
    const session = await getSession();
    if (!session.isAuthenticated || !session.user) {
      return {
    ...
    // Verify request exists and user has access
    const request = await requestRepository.findById(comment.getRequestId().getValue());
    if (!request) {
      return {
        success: false,
        error: 'Request not found',
    ...
    return {
      success: false,
      error: error instanceof Error ? error.message : 'Failed to get comment thread',
    };
  }
}